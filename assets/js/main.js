...
import { SPOTS, VIDEO_DB, RIVERS, MOUNTAINS, RESERVOIR } from "./spots.js";

/* ── 投影：经纬度 → 场景平面坐标 ────────────────── */
const LON0 = 116.46, LAT0 = 40.25, SCALE = 48;
const KLON = Math.cos((40.25 * Math.PI) / 180);
...
controls.screenSpacePanning = false; // 平移锁定在地面 xz 平面，高度不变
...
// 鼠标滚轮：上下拨动绕视野中心水平旋转 3D 模型（方位角变化，高度与距离不变）
...
function declutterCards() {
  ...
  for (const n of items) {
    ...
    n.ppu = fovScale / dist; // 每世界单位对应像素数
    n.halfW = (n.sprite.scale.x / 2) * n.ppu;
    n.halfH = (n.sprite.scale.y / 2) * n.ppu;
    n.cy = syBottom - n.halfH; // 卡片锚点在底部，矩形中心在其上方
    n.fx = 0;
    n.fy = 0;
  }

  // 2. 两两检测重叠，沿穿透较小的轴互相推开
  ...
  // 3. 像素位移换算为沿相机右/上方向的世界偏移，平滑应用；无重叠时缓慢归位
...
let videoPanelSpot = null; // 当前展示视频的景点
let panelPinned = false;   // 点击景点照片后固定面板
...
function showVideoPanel(spot) {
  ...
  vids.forEach((v) => {
    ...
    item.innerHTML = `
      <div class="vp-thumb">
        <img src="assets/img/yt/${v.id}.jpg"
             onerror="this.onerror=null;this.src='https://i.ytimg.com/vi/${v.id}/hqdefault.jpg'" alt="">
...
// 点击景点照片：固定显示该景区的 YouTube 视频面板（再点空白处关闭）
// 拖拽平移结束产生的 click 忽略，避免误关面板
let downX = 0, downY = 0;
...
function flyTo(id) {
  ...
  if (!node) return;
  // 水平位移：目标点平移到视野中心，相机同步平移，俯角与高度不变
...
async function init() {
  const res = await fetch("assets/data/beijing.json");
